...
class TouristPage extends StatefulWidget {
  TouristPage({Key key}) : super(key: key);

  @override
  TouristPageState createState() => TouristPageState();
}


class TouristPageState extends State<TouristPage> {
  StateModel appState;
    ...
    return new DefaultTabController(
      ...
      child: new Scaffold (
        appBar: new AppBar(
          ...
          bottom: new TabBar(
            tabs: [
              Tab(icon: Icon(Icons.directions_walk), child: new Text("My Bookings/Chats")),
        ...
        body: new TabBarView(
          children: <Widget>[
            _buildStreamBuilder(toursBooked),
  ...
  Widget _buildStreamBuilder(List toursList) {
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) => 
              ...
              _buildListItem(context, toursList[index])
          );
        }
      );
  }
...
//////////////////////////////////////////////////////////////////////////////
  ...
  void getBookedTours() async{
    List temp = new List();
    
    FirebaseUser user = await FirebaseAuth.instance.currentUser();
    this.userID = user.uid;
    QuerySnapshot doc = await Firestore.instance.collection('TourBooking').where('userID',isEqualTo:userID).getDocuments();
    for (int i = 0; i < doc.documents.length; i++){
        var tour = doc.documents[i];
        temp.add(tour);
    }

    //get the tour list data according to booked tours
